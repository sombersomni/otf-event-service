...
def get_owner_ids(task_type_name):
    try:
        owner_ids = db.session.query(OwnerTaskAssociation.owner_id).join(OwnerTaskAssociation.task_type).filter(TaskType.name == task_type_name).all()
    except Exception as e:
        return [], 'Cannot find owner ids for the task type selected'
    return (owner_id for owner_id in owner_ids), None
...
if __name__ == '__main__':
    owner_ids, err = get_owner_ids('game-feed')
    if err is None:
      print(list(owner_ids))
    # asyncio.run(job())